...
export class AddcarsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.addmode = true;
    this.editmode = false;
    this.CarsService.listCars().subscribe(data => {
      if (data['success']) {
  ...
  addCars() {
    ...
    this.CarsService.addCars(this.CarsForm.value).subscribe(data => {
      if (data['success']) {
        this.recall();
        this.showSuccess(data['message']);
      } else {
  ...
  updateCars() {
    this.CarsService.updateCars(this.CarsForm.value).subscribe(data => {
      if (data['success']) {
        this.recall();
        this.showSuccess(data['message']);
      } else {
  ...
  deleteCars() {
    ...
    for (let i = 0; i < selectedData.length; i++) {
      this.CarsService.deleteCars(selectedData[i].uid).subscribe(data => {
        if (data['success']) {
          this.showSuccess(data['message']);
        } else {
          this.showError(data['message']);
