...
@Component({
  selector: 'app-admin-quote-status',
  templateUrl: './admin-quote-status.component.html',
  styleUrls: ['./admin-quote-status.component.scss']
})
export class AdminQuoteStatusComponent implements OnInit {
  dtOptions: DataTables.Settings = {};
  QuotesStatusList: any;
  dataLoaded: boolean;
  addmode: boolean;
  editmode: boolean;
  listmode: boolean;
  selectedData_ID: any;
  addQuotesStatusForm: FormGroup;
  deletemode: boolean;
  imageUploaded: boolean;
  QuotesStatusUrl: string;
  ...
  constructor(private adminService: AdminModuleService, private router: Router,
    ...
    this.addQuotesStatusForm = this.formBuilder.group({
      id: [''],
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'quote_status_id') {
    ...
    else if (selectedVal == 'quote_status_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['quote_status', this.myForm.value.arr]).subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.QuotesStatusList = await data['data'];
  ...
  addQuotesStatus() {
    this.router.navigateByUrl('admin/status-management/quote/add/0');
  }
  ...
  editQuotesStatus(data) {
    this.router.navigateByUrl('admin/status-management/quote/update/' + data.quote_status_id);
  }


  deleteQuotesStatus() {
    this.dataLoaded = false;
    this.adminService.deleteQuoteStatus(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/status-management/quote/view/' + data.quote_status_id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/status-management/quote/clone/' + data.quote_status_id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "quote_status").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.QuotesStatusList = await data['data'];
        this.dataLoaded = true;
