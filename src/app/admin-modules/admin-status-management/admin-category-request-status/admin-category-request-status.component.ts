...
@Component({
  selector: 'app-admin-category-request-status',
  templateUrl: './admin-category-request-status.component.html',
  styleUrls: ['./admin-category-request-status.component.scss']
})
export class AdminCategoryRequestStatusComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'cat_req_status_id') {
    ...
    else if (selectedVal == 'cat_req_status_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['category_request_status', this.myForm.value.arr]).subscribe(async data => {
  ...
  addCategoryRequestStatus() {
    this.router.navigateByUrl('admin/status-management/category-request/add/0');
