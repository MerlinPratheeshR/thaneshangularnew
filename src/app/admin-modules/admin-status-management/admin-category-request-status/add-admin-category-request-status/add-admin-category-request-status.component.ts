...
@Component({
  selector: 'app-add-admin-category-request-status',
  templateUrl: './add-admin-category-request-status.component.html',
  styleUrls: ['./add-admin-category-request-status.component.scss']
})
export class AddAdminCategoryRequestStatusComponent implements OnInit {
  ...
  constructor(private adminService: AdminModuleService, private router: Router, private _Activatedroute: ActivatedRoute,
    ...
    this.addCategoryRequestStatusForm = this.formBuilder.group({
      cat_req_status_id: [''],
      cat_req_status_name: ['', [Validators.required, Validators.pattern(this.special_char)]],
  ...
  ngOnInit(): void {
    ...
    this._Activatedroute.paramMap.subscribe(params => {
      ...
      if (this.type == 'update') {
        ...
        this.adminService.geCategoryRequestStatusInfo(this.id).subscribe(data => {
          if (data['success']) {
            this.addCategoryRequestStatusForm.patchValue({
              cat_req_status_id: data['data'].cat_req_status_id,
              cat_req_status_name: data['data'].cat_req_status_name,
        ...
        this.editmode = false;
        this.addmode = false;
        this.addCategoryRequestStatusForm.disable()
        this.adminService.geCategoryRequestStatusInfo(this.id).subscribe(data => {
          if (data['success']) {
            this.addCategoryRequestStatusForm.patchValue({
              cat_req_status_id: data['data'].cat_req_status_id,
  ...
  addCategoryRequestStatus() {
    if (this.addCategoryRequestStatusForm.valid &&
      this.addCategoryRequestStatusForm.controls.cat_req_status_name.value.trim() != ""
      && this.addCategoryRequestStatusForm.controls.remarks.value.trim() != "") {
  ...
  cancel() {
    ...
    this.onPageReload();
    this.router.navigateByUrl('admin/status-management/category-request')
  }
