...
@Component({
  selector: 'app-admin-shipment-status',
  templateUrl: './admin-shipment-status.component.html',
  styleUrls: ['./admin-shipment-status.component.scss']
})
export class AdminShipmentStatusComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'shipment_status_id') {
    ...
    else if (selectedVal == 'shipment_status_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['shipment_status', this.myForm.value.arr]).subscribe(async data => {
  ...
  addShipmentStatus() {
    this.router.navigateByUrl('admin/status-management/shipment/add/0');
  }
  ...
  editShipmentStatus(data) {
    this.router.navigateByUrl('admin/status-management/shipment/update/' + data.shipment_status_id);
  }


  deleteShipmentStatus() {
    this.dataLoaded = false;
    this.adminService.deleteShipmentStatus(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/status-management/shipment/view/' + data.shipment_status_id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/status-management/shipment/clone/' + data.shipment_status_id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "shipment_status").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.ShipmentStatusList = await data['data'];
        this.dataLoaded = true;
