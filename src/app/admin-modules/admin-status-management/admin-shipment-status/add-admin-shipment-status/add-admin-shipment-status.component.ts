...
export class AddAdminShipmentStatusComponent implements OnInit {
  ...
  constructor(private adminService: AdminModuleService, private router: Router, private _Activatedroute: ActivatedRoute,
    ...
    this.addShipmentStatusForm = this.formBuilder.group({
      shipment_status_id: [''],
      shipment_status_name: ['', [Validators.required, Validators.pattern(this.special_char)]],
  ...
  ngOnInit(): void {
    ...
    this._Activatedroute.paramMap.subscribe(params => {
      ...
      if (this.type == 'update') {
        ...
        this.editmode = false;
        this.addmode = false;
        this.addShipmentStatusForm.disable();
        this.adminService.geShipmentStatusInfo(this.id).subscribe(data => {
          if (data['success']) {
            this.addShipmentStatusForm.patchValue({
              shipment_status_id: data['data'].shipment_status_id,
  ...
  addShipmentStatus() {
    if (this.addShipmentStatusForm.valid &&
      this.addShipmentStatusForm.controls.shipment_status_name.value.trim() != ""
      && this.addShipmentStatusForm.controls.remarks.value.trim() != "") {
