...
@Component({
  ...
  styleUrls: ['./admin-order-status.component.scss']
})
export class AdminOrderStatusComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'order_status_id') {
    ...
    else if (selectedVal == 'order_status_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['order_status', this.myForm.value.arr]).subscribe(async data => {
  ...
  addOrderStatus() {
    this.router.navigateByUrl('admin/status-management/order/add/0');
  }
  ...
  editOrderStatus(data) {
    this.router.navigateByUrl('admin/status-management/order/update/' + data.order_status_id);
  }


  deleteOrderStatus() {
    this.dataLoaded = false;
    this.adminService.deleteOrderStatus(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/status-management/order/view/' + data.order_status_id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/status-management/order/clone/' + data.order_status_id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "order_status").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.OrderStatusList = await data['data'];
        this.dataLoaded = true;
