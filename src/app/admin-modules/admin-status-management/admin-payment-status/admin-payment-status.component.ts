...
@Component({
  selector: 'app-admin-payment-status',
  templateUrl: './admin-payment-status.component.html',
  styleUrls: ['./admin-payment-status.component.scss']
})
export class AdminPaymentStatusComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'payment_status_id') {
    ...
    else if (selectedVal == 'payment_status_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['payment_status', this.myForm.value.arr]).subscribe(async data => {
  ...
  addPaymentStatus() {
    this.router.navigateByUrl('admin/status-management/payment/add/0');
  }
  ...
  editPaymentStatus(data) {
    this.router.navigateByUrl('admin/status-management/payment/update/' + data.payment_status_id);
  }


  deletePaymentStatus() {
    this.dataLoaded = false;
    this.adminService.deletePaymentStatus(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/status-management/payment/view/' + data.payment_status_id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/status-management/payment/clone/' + data.payment_status_id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "payment_status").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.PaymentStatusList = await data['data'];
        this.dataLoaded = true;
