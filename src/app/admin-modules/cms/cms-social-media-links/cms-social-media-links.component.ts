...
export class CmsSocialMediaLinksComponent implements OnInit {
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['social_media_links_cms', this.myForm.value.arr]).subscribe(async data => {
  ...
  getMediaLinksList() {
    this.adminService.getMediaLinksList().subscribe(async data => {
      if (data['success']) {
        this.getMediaLinksList = await data['data'];
        this.dataLoaded = true;
      } else {
        this.medialinksList = [];
      }
  ...
  addMediaLinks() {
    ...
    let media_link_count = this.medialinksList.filter(item => item.status == true);
    if (media_link_count.length <= 5) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/cms/social-media-links/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/cms/social-media-links/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "social_media_links_cms").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.medialinksList = await data['data'];
        this.dataLoaded = true;
