...
export class CmsBannerComponent implements OnInit {
  ...
  UploadBanner(event) {
    this.adminService.uploadBannerCms(event.target.files[0]).subscribe(async data => {
      if (data['success']) {
        ...
        this.bannerUrl = await environment.baseurl + "cms-main/banner_cms/viewimage?filename=" + data['uploadedfile'];
        this.addBannerForm.patchValue({
          image: environment.baseurl + "cms-main/banner_cms/viewimage?filename=" + data['uploadedfile']
        });
  ...
  addBanner() {
    this.router.navigateByUrl('admin/cms/banner/add/0');
  }
  ...
  editBanner(data) {
    if (data != null) {
      this.router.navigateByUrl('admin/cms/banner/update/' + data.id);
    } else {
      this.showWarning("Please Select a Record");
    }
  }
  ...
  deleteBanner() {
    this.dataLoaded = false;
    this.adminService.deleteBanner(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/cms/banner/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/cms/banner/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "banner_cms").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.BannerList = await data['data'];
        this.dataLoaded = true;
