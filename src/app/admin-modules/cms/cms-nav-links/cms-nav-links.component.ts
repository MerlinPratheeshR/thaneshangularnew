...
@Component({
  selector: 'app-cms-nav-links',
  templateUrl: './cms-nav-links.component.html',
  styleUrls: ['./cms-nav-links.component.scss']
})
export class CmsNavLinksComponent implements OnInit {
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['nav_links_cms', this.myForm.value.arr]).subscribe(async data => {
  ...
  addNavLinks() {
    this.router.navigateByUrl('admin/cms/nav-links/add/0');
  }



  editNavLinks(data) {
    this.router.navigateByUrl('admin/cms/nav-links/update/' + data.id);
  }
  ...
  deleteNavLinks() {
    this.dataLoaded = false;
    this.adminService.deleteNavLinks(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/cms/nav-links/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/cms/nav-links/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "nav_links_cms").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.NavLinksList = await data['data'];
        this.dataLoaded = true;
