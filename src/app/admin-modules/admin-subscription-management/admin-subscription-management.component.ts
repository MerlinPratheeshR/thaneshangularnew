...
export class AdminSubscriptionManagementComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'id' || selectedVal == 'number_of_users') {
    ...
    else if (selectedVal == 'subscription_name' || selectedVal == 'modules_list') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['subscription_management', this.myForm.value.arr]).subscribe(async data => {
  ...
  addSubscriptionPlan() {
    this.router.navigateByUrl('admin/subscription-management/configure/add/0');
  }
  ...
  editSubscriptionPlan(data) {
    this.router.navigateByUrl('admin/subscription-management/configure/update/' + data.id);
  }
