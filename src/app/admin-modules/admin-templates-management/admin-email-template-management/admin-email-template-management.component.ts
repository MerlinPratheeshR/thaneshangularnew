...
@Component({
  selector: 'app-admin-email-template-management',
  templateUrl: './admin-email-template-management.component.html',
  styleUrls: ['./admin-email-template-management.component.scss']
})
export class AdminEmailTemplateManagementComponent implements OnInit {
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['email_template', this.myForm.value.arr]).subscribe(async data => {
  ...
  addEMAIL() {
    this.router.navigateByUrl('admin/templates-management/email-templates-management/configure/add/0');
  }
  ...
  editEMAIL(data) {
    this.router.navigateByUrl('admin/templates-management/email-templates-management/configure/update/' + data.id);
  }
