...
@Component({
  selector: 'app-admin-configuration-details-management',
  templateUrl: './admin-configuration-details-management.component.html',
  styleUrls: ['./admin-configuration-details-management.component.scss']
})
export class AdminConfigurationDetailsManagementComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    else if (selectedVal == 'config_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['configuration_details', this.myForm.value.arr]).subscribe(async data => {
  ...
  addConfiguration_details() {
    this.router.navigateByUrl('admin/templates-management/configuration-details-management/configure/add/0');
  }
