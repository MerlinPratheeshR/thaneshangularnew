...
@Component({
  selector: 'app-admin-commission-management',
  templateUrl: './admin-commission-management.component.html',
  styleUrls: ['./admin-commission-management.component.scss']
})
export class AdminCommissionManagementComponent implements OnInit {
  dtOptions: DataTables.Settings = {};
  IntermediatorList: any;
  dataLoaded: boolean;
  addmode: boolean;
  editmode: boolean;
  listmode: boolean;
  selectedData_ID: any;
  addIntermediatorForm: FormGroup;
  deletemode: boolean;
  ...
  constructor(private adminService: AdminModuleService, private router: Router, private AdvanceFilterService: AdvanceFilterService,
    ...
    this.addIntermediatorForm = this.formBuilder.group({
      id: [''],
      product_type: ['', Validators.required],
      commission_precentage: ['', Validators.required],
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['commission-management', this.myForm.value.arr]).subscribe(async data => {
      console.log("advanceFilterForm", data);
      if (data['success']) {
        this.IntermediatorList = await data['data'];
  ...
  getUserList() {
    this.adminService.getUserList().subscribe(async data => {
      if (data['success']) {
        ...
        this.IntermediatorList = [];
      }
    });
  }

  getIntermediatorList() {
    this.adminService.getCommissionList().subscribe(async data => {
      if (data['success']) {
        this.IntermediatorList = await data['data'];
        console.log(this.IntermediatorList);
        this.dataLoaded = true;
      } else {
        this.IntermediatorList = [];
      }
  ...
  addIntermediator() {
    this.router.navigateByUrl('admin/commission-management/configure/add/0');
  }
  ...
  editCategoryRequestStatus(data) {
    this.router.navigateByUrl('admin/commission-management/update/' + data.id);
  }


  deleteBuyer() {
    this.dataLoaded = false;
    this.adminService.deleteCommission(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  updateCategoryRequestStatus() {
    this.dataLoaded = false;
    this.adminService.updateCategoryRequestStatus(this.addIntermediatorForm.value).subscribe(data => {
      if (data['success']) {
        this.showSuccess(data['msg']);
        this.cancel();
        this.selectedData_ID = null;
  ...
  editFeatures(data) {
    ...
    this.router.navigateByUrl('admin/commission-management/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/commission-management/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "commission-management").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.IntermediatorList = await data['data'];
        this.dataLoaded = true;
