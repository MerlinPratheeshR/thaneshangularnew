...
export class AddpayoutScheduleComponent implements OnInit {
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['payout_shed_details',this.myForm.value.arr]).subscribe(async data => {
  ...
  addPayout(){
    this.router.navigateByUrl('admin/payout-schedule-management/configure/add/0');
  }
  ...
  editFeatures(data){
    this.router.navigateByUrl('admin/payout-schedule-management/configure/view/'+data.id);
  }

  cloneFeatures(data){
    this.router.navigateByUrl('admin/payout-schedule-management/configure/clone/'+data.id);
  }

  editPayout(data){
    this.router.navigateByUrl('admin/payout-schedule-management/configure/update/'+data.id);
  }
  deletePayout(){
    this.dataLoaded = false;
  ...
  getData(){
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value,"payout_shed_details").subscribe(async data=>{
      console.log("advanceFilterForm",data)
      if (data['success']) {
        this.PayoutList = await data['data'];
        this.dataLoaded = true;
