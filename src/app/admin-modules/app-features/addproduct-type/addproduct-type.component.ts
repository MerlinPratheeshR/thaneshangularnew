...
export class AddproductTypeComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    else if (selectedVal == 'product_type_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['product_type_details', this.myForm.value.arr]).subscribe(async data => {
  ...
  addProductType() {
    this.router.navigateByUrl('admin/product-type-management/configure/add/0');
  }
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/product-type-management/configure/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/product-type-management/configure/clone/' + data.id);
  }

  editProductType(data) {
    this.router.navigateByUrl('admin/product-type-management/configure/update/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "product_type_details").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.ProductTypeList = await data['data'];
        this.dataLoaded = true;
