...
export class AddroleComponent implements OnInit {
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['roles', this.myForm.value.arr]).subscribe(async data => {
  ...
  addRole() {
    this.router.navigateByUrl('admin/role-management/configure/add/0');
  }
  ...
  editRole(data) {
    this.router.navigateByUrl('admin/role-management/configure/update/' + data.id);
  }


  deleteRole() {
    this.dataLoaded = false;
    this.adminService.deleteRole(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  updateRole() {
    this.dataLoaded = false;
    this.adminService.updateRole(this.addRoleForm.value).subscribe(data => {
      if (data['success']) {
        this.showSuccess(data['msg']);
        this.cancel();
        this.selectedData_ID = null;
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/role-management/configure/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/role-management/configure/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "roles").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.RoleList = await data['data'];
        this.dataLoaded = true;
