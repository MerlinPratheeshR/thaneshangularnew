...
export class AdminAddRoleComponent implements OnInit {
  ...
  constructor(private adminService: AdminModuleService,
    ...
    this.addRoleForm = this.formBuilder.group({
      id: [''],
      role_name: ['', [Validators.required, Validators.pattern(this.special_char)]],
      role_code: ['', [Validators.required, Validators.pattern(this.special_char)]],
      action: ['', [Validators.required, Validators.pattern(this.special_char)]],
  ...
  ngOnInit(): void {
    ...
    this._Activatedroute.paramMap.subscribe(params => {
      ...
      if (this.type == 'update') {
        ...
        this.editmode = true;
        this.deletemode = false;
        this.adminService.getRoleInfo(this.id).subscribe(data => {
          if (data['success']) {
  ...
  addRole() {
    if (this.addRoleForm.valid && this.addRoleForm.controls.role_code.value.trim() != ""
      && this.addRoleForm.controls.action.value.trim() != "" &&
      this.addRoleForm.controls.role_name.value.trim() != "") {
      console.log(this.addRoleForm.value);
      this.adminService.createRole(this.addRoleForm.value).subscribe(data => {
        if (data['success']) {
