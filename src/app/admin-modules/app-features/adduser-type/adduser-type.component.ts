...
export class AdduserTypeComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    if (selectedVal == 'login_user_type_id') {
    ...
    else if (selectedVal == 'login_user_type_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['login_user_type', this.myForm.value.arr]).subscribe(async data => {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/user-type-management/configure/view/' + data.login_user_type_id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/user-type-management/configure/clone/' + data.login_user_type_id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "login_user_type").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.UserTypeList = await data['data'];
        this.dataLoaded = true;
