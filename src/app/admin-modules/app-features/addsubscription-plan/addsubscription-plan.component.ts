...
export class AddsubscriptionPlanComponent implements OnInit {
  ...
  onOptionsSelected(selectedVal: any, i: any) {
    ...
    else if (selectedVal == 'subscription_name') {
      $("#ddlstatus" + i).css('display', 'none');
      $("#ddlcreatedBy" + i).css('display', 'none');
      $("#numberinput" + i).css('display', 'block');
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['subscription_details', this.myForm.value.arr]).subscribe(async data => {
  ...
  addSubscriptionPlan() {
    this.router.navigateByUrl('admin/subscription-plan-management/configure/add/0');
  }
  ...
  editSubscriptionPlan(data) {
    this.router.navigateByUrl('admin/subscription-plan-management/configure/update/' + data.id);
  }
  deleteSubscriptionPlan() {
    this.dataLoaded = false;
    this.adminService.deleteSubscriptionPlan(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    this.router.navigateByUrl('admin/subscription-plan-management/configure/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/subscription-plan-management/configure/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "subscription_details").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.SubscriptionPlanList = await data['data'];
        this.dataLoaded = true;
