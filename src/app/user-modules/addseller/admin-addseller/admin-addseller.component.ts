...
@Component({
  selector: 'app-admin-addseller',
  templateUrl: './admin-addseller.component.html',
  styleUrls: ['./admin-addseller.component.scss']
})
export class AdminAddsellerComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private router: Router, private _Activatedroute: ActivatedRoute, private adminService: AdminModuleService, private SellersService: SellersService,
  ...
  ngOnInit(): void {
    ...
    this._Activatedroute.paramMap.subscribe(params => {
      ...
      if (this.type == 'update') {
        ...
        this.SellersService.detailsBuyers(this.id).subscribe(data => {
          console.log(data);
          if (data['success']) {
    ...
    this.SellersService.listBuyers().subscribe(data => {
      if (data['success']) {
  ...
  onFocusOut(email): void {
    if (email != null && email != undefined) {
      this.SellersService.emailIdCheck(email).subscribe(async data => {
        if (data['success']) {
  ...
  onOptionsSelected(selectedVal: any) {
    ...
    this.SellersService.createId().subscribe(async data => {
      if (data['success']) {
      ...
      console.log('SEL' + twoDigitYear + selectedVal + this.newbyerId.id);

      this.BuyersForm.controls.iv_company_id.setValue('SEL' + twoDigitYear + selectedVal + this.newbyerId.id);
  ...
  deleteSellers() {
    ...
    for (let i = 0; i < selectedData.length; i++) {
      this.SellersService.deleteBuyers(selectedData[i].uid).subscribe(data => {
        if (data['success']) {
          this.showSuccess(data['message']);
        } else {
          this.showError(data['message']);
  ...
  addSeller() {
    ...
    /*if (this.BuyersForm.valid) {
      this.SellersService.addBuyers(this.BuyersForm.value).subscribe(data => {
        if (data['success']) {
...
*/
    ...
    if (this.BuyersForm.valid) {
      this.router.navigateByUrl('admin/sellermanagement');
  ...
  updateBuyer() {
    ...
    if (this.BuyersForm.valid) {
      ...
      this.SellersService.updateBuyers(this.BuyersForm.value).subscribe(data => {
        if (data['success']) {
          this.showSuccess(data['msg']);
          this.cancel();
        } else {
