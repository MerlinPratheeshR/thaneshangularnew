...
export class AddintermediatorComponent implements OnInit {
  ...
  constructor(private adminService: AdminModuleService, private intermedaitorService: IntermedaitorService, private router: Router, private AdvanceFilterService: AdvanceFilterService,
  ...
  AdvancesFilterData() {
    ...
    this.dataLoaded = false;
    this.adminService.searchAdvanceFilter(['intermediator', this.myForm.value.arr]).subscribe(async data => {
  ...
  getIntermediatorList() {
    this.intermedaitorService.listIntermedaitor().subscribe(async data => {
      if (data['success']) {
        this.IntermediatorList = await data['data'];
  ...
  editCategoryRequestStatus(data) {
    this.router.navigateByUrl('admin/intermediator/update/' + data.id);
  }


  deleteBuyer() {
    this.dataLoaded = false;
    this.intermedaitorService.deleteIntermedaitor(this.selectedData_ID).subscribe(data => {
      if (data['success']) {
  ...
  editFeatures(data) {
    ...
    this.router.navigateByUrl('admin/intermediator/view/' + data.id);
  }

  cloneFeatures(data) {
    this.router.navigateByUrl('admin/intermediator/clone/' + data.id);
  }
  ...
  getData() {
    this.dataLoaded = false;
    this.adminService.searchFeaturesList(this.advanceFilterForm.value, "intermediator").subscribe(async data => {
      console.log("advanceFilterForm", data)
      if (data['success']) {
        this.IntermediatorList = await data['data'];
        this.dataLoaded = true;
